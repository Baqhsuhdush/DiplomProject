    ...
    <AuthGate title={t("newGoal.title")}>
      <div className="mx-auto max-w-lg px-4 py-10 text-zinc-100">
        <div className="mb-6">
          ...
          <h1 className="mt-4 text-2xl font-semibold">{t("newGoal.title")}</h1>
        </div>

        <form onSubmit={onSubmit} className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5">
          <label className="block text-sm">
            <span className="text-zinc-400">{t("newGoal.domain")}</span>
            <select
              required
          ...
          <label className="block text-sm">
            <span className="text-zinc-400">{t("newGoal.formulation")}</span>
            <textarea
              required
              minLength={1}
              rows={3}
          ...
          <label className="block text-sm">
            <span className="text-zinc-400">{t("newGoal.date")}</span>
            <input
              type="date"
              className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 outline-none ring-emerald-500/30 focus:ring-2"
              value={targetDate}
          ...
          <label className="block text-sm">
            <span className="text-zinc-400">{t("newGoal.priority")}</span>
            <input
              type="number"
              min={1}
              max={5}
